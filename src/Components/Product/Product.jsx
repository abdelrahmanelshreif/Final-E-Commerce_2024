...
export default function Product({ item }) {
  ...
  if (!item) {
    return <div>Loading...</div>; // Handle the case when item is null or undefined
  }
      ...
      <div className="product cursor-pointer p-3 rounded-3 mb-2">
        <div className="position-relative">
          <button
            ...
            className="position-absolute top-0 end-0 fs-3 border border-0 bg-transparent z-3"
          >
            {style ? (
        ...
        <Link to={"/product-details/" + item._id}>
          <img src={item.imageCover} className="w-100 mb-2" alt={item.title} />
          <span className="text-main fw-semibold fs-6">
